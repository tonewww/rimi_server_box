...
class AsyncSSHClient {
  ...
  Stream<String> get stdout => _stdoutController?.stream ?? const Stream.empty();
  Stream<String> get stderr => _stderrController?.stream ?? const Stream.empty();
  Stream<SSHConnectionStatus> get status => _statusController?.stream ?? const Stream.empty();
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      // Start connection
      final response = await _sendMessage(SSHMessage(
        id: _nextMessageId(),
        type: SSHMessageType.connect,
      ));
  ...
  Future<void> shell({
    ...
    logger.info('Opening shell session (term=$termType, cols=$cols, rows=$rows)');
  ...
  Future<void> disconnect() async {
    ...
    try {
      if (_sendPort != null) {
        await _sendMessage(SSHMessage(
          id: _nextMessageId(),
          type: SSHMessageType.disconnect,
  ...
  void _handleIsolateMessage(dynamic message) {
    if (message is SendPort) {
      _sendPort = message;
      _responseHandlers[-1]?.complete(SSHMessage(id: -1, type: SSHMessageType.ready));
    ...
    if (message is Map) {
      ...
      switch (sshMessage.type) {
        ...
        case SSHMessageType.status:
          final status = SSHConnectionStatus.values[sshMessage.data?['status'] as int? ?? 0];
...
class SSHIsolate {
      ...
      try {
        switch (sshMessage.type) {
          ...
          case SSHMessageType.execute:
            final output = await _execute(sshMessage.data?['command'] as String);
            _reply(sshMessage, SSHMessage(
              id: sshMessage.id,
              type: SSHMessageType.response,
              data: {'output': output},
  ...
  Future<void> _authenticateWithKey() async {
    ...
    privateKeyPtr.asTypedList(privateKeyBytes.length).setAll(0, privateKeyBytes);
